...
class Coin extends Component {
  render() {
    ...
    // support both /coin/:symbol and /coin/* wildcard
    const paramFromRouter =
      (this.props.match &&
        this.props.match.params &&
        (this.props.match.params.symbol || this.props.match.params["0"])) ||
      null;
    const coin =
      paramFromRouter ||
    ...
    const volume24 =
      Boolean(marketData[coin] && marketData[coin].ticker) &&
      marketData[coin].ticker.volume * price;
    // console.log(marketData[coin].ticker.volume, 'voluem?');

    const chartColor = !price || price >= cost_basis ? "#21ce99" : "#d82d2d";
    const headStyle = {
    ...
    const changeSuffix =
      "&merchant_id=db206cefa712&address=&amount=1&ref_id=db206cefa712&color=" +
      chartColor.replace("#", "");

    const changellyBuy =
      changePrefix + "from=BTC&to=" + coin.toUpperCase() + changeSuffix;
    const changellySell =
      changePrefix + "from=" + coin.toUpperCase() + "&to=BTC" + changeSuffix;
        ...
        <div className="coinCard">
          ...
          {changellyCoins.includes(coin.toUpperCase()) && (
            <div className="listCoin">
              <a target="_blank" rel="noopener noreferrer" href={changellyBuy}>
                <button className="btn-buy" style={headStyle}>
                  <i className="fa fa-money" aria-hidden="true"></i> Buy
                </button>
              </a>
              <a target="_blank" rel="noopener noreferrer" href={changellySell}>
                <button className="btn-buy" style={headStyle}>
          ...
          {!changellyCoins.includes(coin.toUpperCase()) && (
            <div className="listCoin">
              <a
                ...
                href={
                  "https://changelly.com/widget/v1?auth=email&from=ETH&to=BTC&merchant_id=db206cefa712&address=&amount=1&ref_id=db206cefa712&color=" +
                  chartColor.replace("#", "")
                }
              >
                <button className="btn-buy" style={headStyle}>
                  <i className="fa fa-line-chart" aria-hidden="true"></i> Trade
                  Coins
                </button>
              </a>
            </div>
          ...
          <div className="listCoin">
            ...
            <span className="right">
              ...
              <br />
              <span className="lightGray">
                {string.total + curSymbol + string.holding}
              </span>
            </span>
          </div>
          ...
          <div className="listCoin">
            <span className="left">
              {curSymbol}
              {volume24 && $numberWithCommas(volume24.toFixed())}
              <br />
              <span className="lightGray">{string.volume}</span>
            </span>
            <span className="right">
              {curSymbol}
              {$numberWithCommas(totalCostBasis.toFixed(2))}&nbsp;
              <span className="lightGray">
                ({$numberWithCommas(cost_basis.toFixed(2))})
              </span>
              <br />
          ...
          <div className="trash listCoin">
            ...
            <span className="right">
              <span
                onClick={() => this.props.deleteCoin(coin, this.props.history)}
                className="lightGray"
              >
